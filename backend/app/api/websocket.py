...
class ConnectionManager:
    ...
    async def connect(
        ...
        if len(self._connections) >= self.MAX_TOTAL_CONNECTIONS:
            logger.warning(f"Global WebSocket connection limit reached ({self.MAX_TOTAL_CONNECTIONS})")
            await websocket.accept()
            await websocket.send_json({
                "type": "error",
                "data": {"message": "Server connection limit reached. Please try again later."}
            })
        ...
        if user_id:
            ...
            if existing >= self.MAX_CONNECTIONS_PER_USER:
                logger.warning(f"Per-user WebSocket limit reached for user {user_id} ({existing}/{self.MAX_CONNECTIONS_PER_USER})")
                await websocket.accept()
                await websocket.send_json({
                    "type": "error",
                    "data": {"message": "Too many connections. Please close other tabs first."}
                })
                await websocket.close(code=4003, reason="Per-user connection limit reached")
                return None
    ...
    async def subscribe(
        ...
        await self.send_to_connection(
            connection_id,
            WSMessage(type=MessageType.SUBSCRIBED, channel=channel)
        )
    ...
    async def unsubscribe(
        ...
        await self.send_to_connection(
            connection_id,
            WSMessage(type=MessageType.UNSUBSCRIBED, channel=channel)
        )
