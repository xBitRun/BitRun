        ...
        if token_data.jti:
            try:
                ...
                if await redis.is_token_blacklisted(token_data.jti):
                    logger.warning(f"WebSocket connection attempted with blacklisted token: {token_data.jti[:8]}...")
                    return None
            except Exception as e:
                settings = get_settings()
                if settings.environment == "production":
                    # Fail-closed in production: reject token if we can't verify blacklist
                    logger.error(f"Redis unavailable for token blacklist check in production, rejecting connection: {e}")
                    return None
                else:
                    # Fail-open in development: allow connection if Redis is down
                    logger.warning(f"Redis unavailable for token blacklist check (dev mode, allowing): {e}")
