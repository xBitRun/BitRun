...
class DataAccessLayer:
    ...
    async def get_market_context(
        ...
        kline_tasks = {
            tf: self._get_klines_cached(symbol, tf)
    ...
    async def get_market_contexts(
        ...
        """
        tasks = {
            symbol: self.get_market_context(symbol, timeframes)
    ...
    async def _get_klines_cached(
        ...
        """
        cache_key = f"{self.KLINE_CACHE_PREFIX}{self.trader.exchange_name}:{symbol}:{timeframe}"
    ...
    async def get_indicators_multi(
        ...
        tasks = {
            tf: self.get_indicators(symbol, tf)
            for tf in timeframes
        }
    ...
    async def get_cache_stats(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "total_entries": len(kline_keys) + len(funding_keys) + len(indicator_keys),
