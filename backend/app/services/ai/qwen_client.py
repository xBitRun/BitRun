"""
Qwen (Alibaba) Client Adapter.

Implements the BaseAIClient interface for Alibaba's Qwen models.
...
# Qwen API base URL (DashScope OpenAI-compatible endpoint)
...
class QwenClient(BaseAIClient):
    ...
    Uses OpenAI-compatible API with Alibaba's DashScope endpoint.
    Supports Qwen series models including Qwen-235B.

    Usage:
        config = AIClientConfig(api_key="...", model="qwen-plus")
    ...
    def __init__(self, config: AIClientConfig):
        ...
        # Set Qwen base URL if not custom
        if not config.base_url:
            config.base_url = QWEN_BASE_URL
    ...
    async def generate(
        ...
        Args:
            ...
            json_mode: If True, use JSON mode (supported by Qwen)
