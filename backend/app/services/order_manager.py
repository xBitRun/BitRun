...
class OrderManager:
            ...
            while order.is_open:
                ...
                try:
                    ...
                    if updated is None:
                        ...
                        logger.warning(
                            f"Order {order.order_id} not found, stopping tracking"
                        )
                        break
    ...
    async def _handle_status_change(
        ...
        if order.status == OrderStatus.FILLED:
            ...
            if order.can_retry:
                logger.info(
                    f"Retrying order {order.order_id} (attempt {order.retry_count + 1})"
                )
                ...
                try:
                    await self._maybe_await(
                        self.callback.on_error(
                            order, order.error_message or "Unknown error"
                        )
                    )
                except Exception as e:
