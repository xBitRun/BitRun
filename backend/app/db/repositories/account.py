...
class AccountRepository:
    ...
    async def create(
        ...
        account = ExchangeAccountDB(
            ...
            encrypted_api_secret=self.crypto.encrypt(api_secret) if api_secret else None,
    ...
    async def get_by_user(
        ...
        """
        query = select(ExchangeAccountDB).where(
            ExchangeAccountDB.user_id == user_id
        )
        if exchange:
    ...
    async def get_decrypted_credentials(
        ...
        try:
            ...
            if account.encrypted_api_secret:
                credentials["api_secret"] = self.crypto.decrypt(account.encrypted_api_secret)
