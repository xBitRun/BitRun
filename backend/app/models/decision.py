...
class RiskControls(BaseModel):
    ...
    max_leverage: int = Field(
        default=5, ge=1, le=50, description="Maximum allowed leverage multiplier"
    )
    ...
    max_total_exposure: float = Field(
        default=0.8, ge=0.1, le=1.0, description="Max total exposure as ratio of equity"
    )
    min_risk_reward_ratio: float = Field(
        default=2.0, ge=1.0, description="Minimum risk/reward ratio for trades"
    )
...
class TradingDecision(BaseModel):
    ...
    action: ActionType = Field(..., description="Trading action to take")
    leverage: int = Field(default=1, ge=1, le=50, description="Leverage multiplier")
    ...
    entry_price: Optional[float] = Field(
        None, description="Entry price for limit orders"
    )
    stop_loss: Optional[float] = Field(None, description="Stop loss price")
    take_profit: Optional[float] = Field(None, description="Take profit price")
    confidence: int = Field(..., ge=0, le=100, description="Confidence score 0-100")
    risk_usd: float = Field(
        default=0, ge=0, description="Estimated maximum risk in USD"
    )
    reasoning: str = Field(
        ..., min_length=10, description="Reasoning for this decision"
    )
...
class DecisionResponse(BaseModel):
    ...
    chain_of_thought: str = Field(..., description="AI's reasoning process")
    market_assessment: str = Field(
        ..., description="Overall market condition assessment"
    )
    decisions: list[TradingDecision] = Field(
        default_factory=list, description="List of trading decisions"
    )
    ...
    overall_confidence: int = Field(
        default=50, ge=0, le=100, description="Overall confidence in market conditions"
    )
    next_review_minutes: int = Field(
        default=60, ge=5, le=1440, description="Suggested time until next review"
    )
