...
function StrategyConfigSection({ agent, agentId, t }: { agent: NonNullable<ReturnType<typeof useAgent>["data"]>; agentId: string; t: ReturnType<typeof useTranslations> }) {
  ...
  const aiModelDisplay = agent.ai_model
    ? agent.ai_model.includes(":")
      ...
      : agent.ai_model
    : "-";
  const aiProviderDisplay = agent.ai_model?.split(":")[0] || "";

  // Check which indicators are configured
  const emaPeriods = (indicators.ema_periods as number[]) || [];
  const rsiPeriod = indicators.rsi_period as number | undefined;
  const macdFast = indicators.macd_fast as number | undefined;
...
function SettingsTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <StrategyConfigSection agent={agent} agentId={agentId} t={t} />
