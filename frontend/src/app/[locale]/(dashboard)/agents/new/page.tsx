...
export default function AgentWizardPage() {
  ...
  const { data: boundAccountsInfo = {} } = useBoundAccounts();
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {currentStep === 2 && (
        <ExecutionModeStep
          ...
          setState={setState}
          accounts={accounts ?? []}
          boundAccountsInfo={boundAccountsInfo}
...
function ExecutionModeStep({
  ...
  boundAccountsInfo: Record<string, BoundAccountInfo>;
  ...
  // Get allocation info for selected account
  const selectedAccountAllocation = state.accountId
    ? boundAccountsInfo[state.accountId]
    : undefined;
  const totalPercentUsed = selectedAccountAllocation?.total_percent ?? 0;
  const remainingPercent = 1.0 - totalPercentUsed;

  // Check if selected account is fully allocated (100%)
  const isFullyAllocated =
    selectedAccountAllocation && selectedAccountAllocation.total_percent >= 1.0;

  // Check if percentage allocation exceeds remaining
  const percentAllocationExceeds =
    state.capitalMode === "percent" &&
    state.allocatedCapitalPercent / 100 > remainingPercent;

  // Get allocation mode for selected account
  const accountAllocationMode = selectedAccountAllocation?.allocation_mode;

  // Check if a capital mode option should be disabled
  const isCapitalModeDisabled = (mode: "none" | "fixed" | "percent") => {
    if (mode === "none") return false; // "none" is always allowed
    if (!accountAllocationMode) return false; // No existing mode, all allowed
    return mode !== accountAllocationMode; // Disable if different from existing
  };

  // Check if the currently selected mode conflicts
  const hasAllocationModeConflict =
    state.capitalMode !== "none" &&
    accountAllocationMode &&
    state.capitalMode !== accountAllocationMode;
        ...
        <>
          <Card>
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {accounts.length === 0 ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      {accounts.map((acc) => {
                        const info = boundAccountsInfo[acc.id];
                        const accFullyAllocated =
                          info && info.total_percent >= 1.0;
                        return (
                          <SelectItem
                            key={acc.id}
                            value={acc.id}
                            disabled={accFullyAllocated}
                          >
                            {acc.name} ({acc.exchange})
                            {acc.is_testnet && ` [${t("wizard.testnet")}]`}
                            {info &&
                              ` (${t("wizard.executionStep.allocated")}: ${(info.total_percent * 100).toFixed(0)}%)`}
                          </SelectItem>
                        );
                      })}
                    </SelectContent>
              ...
              {/* Account Already Allocated Info */}
              {selectedAccountAllocation &&
                selectedAccountAllocation.agent_count > 0 &&
                !isFullyAllocated && (
                  <div className="flex items-start gap-2 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                    <AlertTriangle className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
                    <div className="text-xs text-blue-700 dark:text-blue-400">
                      <p className="font-medium">
                        {t("wizard.executionStep.alreadyAllocated")}
                      </p>
                      <p className="mt-1">
                        {t("wizard.executionStep.alreadyAllocatedDesc", {
                          percent: (
                            selectedAccountAllocation.total_percent * 100
                          ).toFixed(0),
                          agents: selectedAccountAllocation.agent_count,
                          remaining: (remainingPercent * 100).toFixed(0),
                        })}
                      </p>
                    </div>
                  </div>
              ...
              {isFullyAllocated && (
                <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                  ...
                  <div className="text-xs text-destructive">
                    <p className="font-medium">
                      {t("wizard.executionStep.fullyAllocated")}
                    </p>
                    <p className="mt-1">
                      {t("wizard.executionStep.fullyAllocatedDesc")}
                    </p>
                  </div>
                </div>
              )}

              {/* Percentage Allocation Exceeded Warning */}
              {percentAllocationExceeds && (
                <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                  <AlertTriangle className="w-4 h-4 text-destructive mt-0.5 shrink-0" />
                  <div className="text-xs text-destructive">
                    <p className="font-medium">
                      {t("wizard.executionStep.allocationExceeded")}
                    </p>
                    <p className="mt-1">
                      {t("wizard.executionStep.allocationExceededDesc", {
                        requested: state.allocatedCapitalPercent.toFixed(0),
                        available: (remainingPercent * 100).toFixed(0),
                      })}
                    </p>
                  </div>
                </div>
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <RadioGroup
                ...
                onValueChange={(v: string) => {
                  // Prevent selecting disabled mode
                  if (isCapitalModeDisabled(v as "none" | "fixed" | "percent"))
                    return;
                  setState((prev) => ({
                    ...prev,
              ...
              >
                ...
                >
                  <RadioGroupItem
                    ...
                    id="capital-fixed"
                    className="sr-only"
                    disabled={isCapitalModeDisabled("fixed")}
                  />
                  <span className="text-sm font-medium">
                  ...
                  {isCapitalModeDisabled("fixed") && (
                    <span className="text-[10px] text-muted-foreground mt-0.5">
                      {t("wizard.capitalAllocation.modeDisabled")}
                    </span>
                  )}
                </Label>
                <Label
                ...
                >
                  <RadioGroupItem
                    ...
                    id="capital-percent"
                    className="sr-only"
                    disabled={isCapitalModeDisabled("percent")}
                  />
                  <span className="text-sm font-medium">
              ...
              {/* Allocation Mode Restriction Info */}
              {accountAllocationMode && (
                <div className="flex items-start gap-2 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <AlertTriangle className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
                  <div className="text-xs text-blue-700 dark:text-blue-400">
                    <p className="font-medium">
                      {t("wizard.capitalAllocation.modeRestriction")}
                    </p>
                    <p className="mt-1">
                      {t("wizard.capitalAllocation.modeRestrictionDesc", {
                        mode:
                          accountAllocationMode === "percent"
                            ? t("wizard.capitalAllocation.modePercent")
                            : t("wizard.capitalAllocation.modeFixed"),
                      })}
                    </p>
